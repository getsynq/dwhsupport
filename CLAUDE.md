...
- **`QueryCustomMetrics` versus `RunRawQuery`**: `QueryCustomMetrics` is the metrics path. It sidelines `segment*` columns into `Segments`, and reads text as a number, a boolean or a timestamp, or else `IgnoredValue`; it never returns `StringValue`, and its consumers (synq-dwh's proto conversion) reject one. `RunRawQuery` keeps every column as its native type, text as `StringValue`, nested cells as `JsonValue`. A helper query that reads a user's values back (a cutoff watermark, bisection keys, a preview) belongs on `RunRawQuery`, whose `Columns()` also carry each column's `Kind` for `SqlLiteral`.
