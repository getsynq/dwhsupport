...
--
-- Descriptions come from sys.extended_properties (MS_Description), as on SQL
-- Server. Fabric currently can't write extended properties so this is NULL
-- today, but the view is queryable (verified live) so the join is free and
-- forward-supports descriptions if Fabric/consumers ever populate them.
SELECT
    s.name                          AS [schema],
    o.name                          AS [table],
    CASE o.type
