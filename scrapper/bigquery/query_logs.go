...
func (s *BigQueryScrapper) FetchQueryLogs(ctx context.Context, from, to time.Time) (querylogs.QueryLogIterator, error) {
...
func (it *bigqueryQueryLogIterator) Next(ctx context.Context) (*querylogs.QueryLog, error) {
	...
	// Convert to QueryLog
	log, err := convertBigQueryRowToQueryLog(&row)
	if err != nil {
...
func convertBigQueryRowToQueryLog(row *BigQueryQueryLogSchema) (*querylogs.QueryLog, error) {
	// Determine status
	status := "SUCCESS"
