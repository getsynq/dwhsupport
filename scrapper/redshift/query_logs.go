...
type redshiftQueryLogIterator struct {
	rows   *sqlx.Rows
	closed bool
}

func (s *RedshiftScrapper) FetchQueryLogs(ctx context.Context, from, to time.Time) (querylogs.QueryLogIterator, error) {
	...
	return &redshiftQueryLogIterator{
		rows: rows,
	}, nil
}

func (it *redshiftQueryLogIterator) Next(ctx context.Context) (*querylogs.QueryLog, error) {
	if it.closed {
		return nil, io.EOF
	}
	...
	// Convert to QueryLog
	log, err := convertRedshiftRowToQueryLog(&row)
	if err != nil {
...
func (it *redshiftQueryLogIterator) Close() error {
	if it.closed {
		return nil
	}
	it.closed = true
...
func convertRedshiftRowToQueryLog(row *RedshiftQueryLogSchema) (*querylogs.QueryLog, error) {
