...
func (e *OracleScrapper) QueryTableMetrics(ctx context.Context, lastMetricsFetchTime time.Time) ([]*scrapper.TableMetricsRow, error) {
	sql := scope.AppendScopeConditions(ctx, queryTableMetricsSql, "", "t.OWNER", "t.TABLE_NAME")
	return dwhexecoracle.NewQuerier[scrapper.TableMetricsRow](e.executor).QueryMany(ctx, sql,
